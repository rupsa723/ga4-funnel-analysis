...
total AS (
    ...
    -- IMPORTANT: COUNT(DISTINCT session_id) to count unique
    -- sessions, not rows — same correction as 03_bounce_rate.sql
    SELECT
        channel,
...
-- Unknown         26,054          23,047            88.46%  ← all real bounces here
-- Self-Referral    1,222               6             0.49%  ← near zero: went deeper
-- Obfuscated         164               2             1.22%  ← same reason
...
-- All bounce traffic is absorbed into 'Unknown' (88.46%).
--
-- The Unknown channel dominates bounced sessions because
-- session_start events — which all bounced sessions consist of
-- — carry no channel information in this dataset.
--
